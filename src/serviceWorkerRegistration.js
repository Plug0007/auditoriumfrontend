...
export function register(config) {
  if (process.env.NODE_ENV === 'production' && 'serviceWorker' in navigator) {
    // Construct the public URL from the PUBLIC_URL environment variable
    const publicUrl = new URL(process.env.PUBLIC_URL, window.location.href);
    if (publicUrl.origin !== window.location.origin) {
      // If PUBLIC_URL is on a different origin, the service worker won't work.
      return;
    }
    ...
    window.addEventListener('load', () => {
      ...
      if (isLocalhost) {
        ...
        // Log additional information on localhost.
        ...
        installingWorker.onstatechange = () => {
          if (installingWorker.state === 'installed') {
            if (navigator.serviceWorker.controller) {
              // New content available; execute onUpdate callback if provided.
